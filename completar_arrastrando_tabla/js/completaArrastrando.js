...
if(esMobil==true)
{
	...
	switch (idioma) {
		...
		default:
			...
			jq321("#btnReiniciar").text("Siguiente intento"); // ic
	}
...
function revisar() {
	...
	if(esMobil==true){
		...
		totalCasillas =  ocupados.length;
		var reqTemp = (respRequeridas ? respRequeridas : casillasPregunta.length);	
		if (ocupados.length < reqTemp) {  //  en lugar de reactivosMostrar pongo respuestas.length, es mas de una respuesta por pregunta....
...
function calificar(){	
	...
	var listaContestadas = jq321 (".ocupado"); //Leo las cajas drop, esto es solo para poner palomas o taches
...
function revisaBuenas() {
	...
	var res = (calificaAciertos ? correctas : calificaAciertos);
	console.log("correctas " + correctas);
	console.log("res " + res);
	console.log("totalCasillas " + totalCasillas);
	switch (idioma) {
		case "ENG":
		...
		if (intentos < maxIntentos) {
			...
			jq321.each(respInc, function (indice) {
				/*			for (i = 0; i < respMal.length; i++) {
							...
							var xx = jq321(this).attr("data-resp");
							if (respRequeridas){
								var tempo = jq321(".droppable.correcto > object[data='"+xx+"']").length;
								if (tempo == 0) {
									jq321(this).attr("style", "position: relative;").css("display", "");
									jq321(this).parent().css("display", "");
								}
