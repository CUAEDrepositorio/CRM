...
function quitarAcentos(str) {
	...
	if (revisionEstricta) {
		...
		var fromSA = "àáäâèéëêìíïîòóöôùúüûñç·/_,:;ABCDEFGHIJKLMNÑOPQRSTUVWXYZ";
		var sinAcentos = "aaaaeeeeiiiioooouuüunc------abcdefghijklmnñopqrstuvwxyz";
		for (var i = 0, l = fromSA.length; i < l; i++) {
			str = str.replace(new RegExp(fromSA.charAt(i), 'g'), sinAcentos.charAt(i));
		}
		str = str.replace(/[^a-z0-9 -]/g, '') // remove invalid chars

	}
	// //Acepta mayusculas las respuestas
	// if (revisionEstricta == false) {
	// str = str.replace(/[^a-z0-9 -]/g, '') // remove invalid chars
